...
/// pincel/linha/retangulo. Tracos sao guardados em coords de imagem (pixel da
/// screenshot), com mirror "desfeito" no clique e refeito no shader — assim
/// o desenho fica preso ao conteudo independente do estado do espelho.
/// </summary>
public sealed class DrawTool
{
                   ...
                   Camera camera, bool mirror)
  {
    ...
    {
      // Linha/retangulo: so 2 pontos (inicio + arrasto atual).
      if (_active.Points.Count < 2) _active.Points.Add(p);
      else _active.Points[1] = p;
    }
  }

  public void End() => _active = null;
